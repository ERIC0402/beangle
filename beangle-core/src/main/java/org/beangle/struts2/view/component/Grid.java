...
public class Grid extends ClosingUIBean {
	...
	private List<ClassPropertyConfig> listables;
	...
	public List<ClassPropertyConfig> getListables() {
		return listables;
	}

	public void setListables(List<ClassPropertyConfig> listables) {
		this.listables = listables;
	}

	public boolean listable(Col col) {
		String name = col.getProperty();
		if(listables == null || StringUtils.isEmpty(name)){
			return true;
		}
//		name = name.substring(name.indexOf(".") + 1);
		...
		for(ClassPropertyConfig cpc : listables){
			if(name.startsWith(cpc.getPropertyName())){
				col.setTitle(cpc.getNewLabel());
				if(cpc.getWidth() != null){
					col.setWidth(cpc.getWidth() + "%");
				}
				return true;
			}
		}
