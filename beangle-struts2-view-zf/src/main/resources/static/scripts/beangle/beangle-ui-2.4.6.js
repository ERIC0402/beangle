...
//			imageRoot=self.location.pathname.substring(0,self.location.pathname.substring(1).indexOf('/')+1)+"/static/themes/"+ bg.uitheme +"/icons/",
		...
		this.addBack = function (title){
			...
			this.addItem(title,function (){
				...
				if(searchBox.length > 0){
					var sinput = searchBox.find(".search_input").first();
					if(sinput.length > 0){
						sinput.click();
						return;
					}else{
		...
		this.addClose = function (msg){
			...
			this.addItem(msg,"window.close()",'ico_close');
		}
		/**
		 * 添加菜单
		 * 
	...
	bg.extend({'ui.loading':function (divId,title,imageName){
		var div = $("#" + divId);
		var img = $("<div style='text-align:center; margin:100px 0;'><img src='" + base + "/static/stylev4/images/ico_loading.gif'><div>");
		div.html("").append(img);
	}});
	...
	function BeangleUiComm(){
		this.addPlaceholder = function (input, msg){
			if(!msg){
				msg = input.attr("alt");
			}
	...
	bg.extend({"ui.comm": new BeangleUiComm()});
	function BeangleUiSearch(){
		this.initForm = function (formId){
			this.initPlaceholder(formId);
			this.initExpandBtn(formId);
			this.reverseItem(formId);
		}
		this.initPlaceholder = function (formId){
			$("#"+ formId +" :input").each(function (){
				var input = $(this);
				bg.ui.comm.addPlaceholder(input);
			});
		}
		this.removePlaceholder = function (formId){
			$("#"+ formId +" :input").each(function (){
				var input = $(this);
		...
		this.addPlaceholder = function (formId){
			this.initPlaceholder(formId);
		}
		this.reverseItem =function (formId){
	...
	bg.extend({"ui.search":new BeangleUiSearch()});
	function BeangleUiList(){
		this.getMainId = function (listId, parentClass){
				if(!parentClass){
	...
	bg.extend({"ui.list": new BeangleUiList()});
	function BeangleUiForm(){
...
//					}
					form.find("h2").filter(".h2_title_1").click(function (){
						var h2 = $(this);
	...
	bg.extend({"ui.form": new BeangleUiForm()});
})(beangle);
