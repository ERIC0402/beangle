...
[@c.tr tag=tag]
    <input type="password" id="${tag.id}" value="${tag.value!}" title="[#if tag.comment??]${tag.comment}[#else]${tag.title!}[/#if]" name="${tag.name}" maxlength="${tag.maxlength}" ${tag.parameterString}  onpaste="return false"/>
[/@]